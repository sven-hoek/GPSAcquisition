...
output reg signed [`DATA_WIDTH-1:0]	RESULT_O;
...
always@(posedge CLK_I)begin
  case (r_counter)
   0: if(w_64_bit_operation) begin
	operand_a_low <= OPERAND_A;
	operand_b_low <= OPERAND_B;
       end 
	else begin
   ...
   1: if(w_64_bit_operation) begin
	operand_a_high <= OPERAND_A;
	operand_b_high <= OPERAND_B;
       end 
	else begin
	operand_a_high <= operand_a_high;
	operand_b_high <= operand_a_high;
...
always@(posedge CLK_I) begin
STATUS_O <= w_status;
CACHE_VALID_O <= w_enable_cache;
CACHE_WRITE_O <= w_write_cache;
end
...
always@(*) begin
  case (OPCODE_I)
    `IADD: 
 	begin 
    w_single_cycle_out = OPERAND_A + OPERAND_B;
    w_status = 1'b0;
    w_enable_cache =1'b0;   
    w_write_cache  =1'b0;   
     end
    ...
    `IFEQ: 
     ...
     w_status = (OPERAND_A == OPERAND_B) ? 1'b1 : 1'b0; 
     w_enable_cache =1'b0;
     w_write_cache  =1'b0;
      end
  `IFNE: 
...
/////////////////////////////////////////////////////
   ...
   `LADD: 
    ...
    if(r_counter[0]) begin
      r_multi_cycle_out = OPERAND_A + OPERAND_B + r_carry;
      w_reminder = 1'b0;
      end
    else begin
       r_multi_cycle_out = {1'b0,OPERAND_A} + {1'b0,OPERAND_B};
      w_reminder = 1'b1;
      end
    end
   `LSUB: 
    ...
    if(r_counter[0]) begin
      ...
      w_reminder = 1'b0;
       ...
       r_multi_cycle_out = {1'b0,OPERAND_A} + {1'b0,~OPERAND_B} +1;
      w_reminder = 1'b1;
      end
    end
// stop
...
always@(posedge CLK_I) begin
RESULT_O <= (w_single_cycle_operation == 1'b0) ? r_multi_cycle_out[`DATA_WIDTH-1:0] : w_single_cycle_out;
end
