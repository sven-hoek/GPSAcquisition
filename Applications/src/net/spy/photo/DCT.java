...
class DCT {
	...
	public int quantum[][] = new int[N*N][2];
	public float Divisors[][] = new float[N*N][2];

	/**
	 * Quantitization Matrix for luminace.
	 */
	public int quantum_luminance[] = new int[N * N];
	public float DivisorsLuminance[] = new float[N * N];

	/**
	 * Quantitization Matrix for chrominance.
	 */
	public int quantum_chrominance[] = new int[N * N];
	public float DivisorsChrominance[] = new float[N * N];
	 ...
	 * initializes the run length counters and the ZigZag sequence. Note
	 * that the image quality can be worse than 25 however the image will be
	...
	private void initMatrix(int q) {
		float[] AANscaleFactor = {
				1.0f, 1.387039845f, 1.306562965f, 1.175875602f, 1.0f, 0.785694958f,
		...
		for(i = 0; i < 8; i++) {
			for(j = 0; j < 8; j++) {
				...
				// jpeg 6a library.
				DivisorsLuminance[index] = (1.0f / (quantum_luminance[index]
						* AANscaleFactor[i] * AANscaleFactor[j] * 8.0f));
				index++;
			}
		}
		...
		for(i = 0; i < 8; i++) {
			for(j = 0; j < 8; j++) {
				...
				// The divisors for the AAN method (the float method used in
				// jpeg 6a library.
				DivisorsChrominance[index] = (1.0f / (quantum_chrominance[index]
						* AANscaleFactor[i] * AANscaleFactor[j] * 8.0f));
				index++;
			}
		}
	 ...
	 * included as a curiosity and can give you an idea of the difference in
	 * the compression result (the resulting image quality) by comparing its
	 * output to the output of the AAN method below. It is ridiculously
	 * inefficient.
	 */

	// For now the final output is unusable. The associated quantization
	// step
	// needs some tweaking. If you get this part working, please let me
	// know.
...
*/
	 ...
	 */
	public int[] quantizeBlock(float inputData[][], int code) {
		int outputData[] = new int[N * N];
		int i, j;
		int index;
		index = 0;
		
//		int [] valueContainer = new int[1000];
//		int cnt = 0;
		for(i = 0; i < 8; i++) {
			for(j = 0; j < 8; j++) {
				...
				float mid = inputData[i][j]* (((Divisors[code]))[index]);
				mid = mid >= 0 ? mid+0.5f : mid -0.5f; 
//				valueContainer[cnt++] = (int)mid;
				outputData[index] = (int) (mid);
...
//			System.out.print(valueContainer[ii]+", ");
//		}System.out.println();
	...
	/*
	 * This is the method for quantizing a block DCT'ed with
	 * forwardDCTExtreme This method quantitizes data and rounds it to the
	 * nearest integer.
	 */
