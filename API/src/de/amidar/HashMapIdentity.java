...
public class HashMapIdentity {
	 ...
	 */
	transient HashEntryIdentity[] buckets;
	 ...
	 */
	static class HashEntryIdentity {
		/**
		...
		public final int hashCode () {
			return (key == null ? 0 : System.identityHashCode (key)) ^ (value == null ? 0 : System.identityHashCode (value));
		}
		 ...
		 *            the value
		 */
		HashEntryIdentity (Object key, Object value) {
			this.key = key;
			this.value = value;
		}
	 ...
	 */
	public HashMapIdentity (int initialCapacity) {
	 ...
	 */
	public HashMapIdentity (int initialCapacity, float loadFactor) {
		if (initialCapacity < 0)
		...
		if (initialCapacity == 0)
			initialCapacity = 1;
		buckets = new HashEntryIdentity[initialCapacity];
	...
	public Object get (Object key) {
		int idx = hash (key);
		HashEntryIdentity e = buckets[idx];
		while (e != null) {
			if (key == e.key)
				return e.value;
			e = e.next;
	...
	public Object put (Object key, Object value) {
		...
		HashEntryIdentity eNew = new HashEntryIdentity (key, value);
	 ...
	 */
	private void rehash () {
		HashEntryIdentity[] oldBuckets = buckets;

		int newcapacity = (buckets.length * 2) + 1;
		threshold = (int) (newcapacity * loadFactor);
		buckets = new HashEntryIdentity[newcapacity];

		for (int i = oldBuckets.length - 1; i >= 0; i--) {
			HashEntryIdentity e = oldBuckets[i];
			while (e != null) {
				int idx = hash (e.key);
				HashEntryIdentity next = e.next;
				e.next = buckets[idx];
				buckets[idx] = e;
				e = next;
			}
	...
	private final class HashIterator implements Iterator {
		...
		private HashEntryIdentity last;
		/**
		 ...
		 */
		private HashEntryIdentity next;
		...
		public void remove () {
			...
			HashMapIdentity.this.remove (last.key);
			last = null;
			knownMod++;
		}
	}
