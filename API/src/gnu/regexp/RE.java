...
// This String will be returned by getVersion()
  ...
  protected void initialize(Object patternObj, int cflags, RESyntax syntax, int myIndex, int nextSub) throws REException {
	...
	int newIndex = getMinMax(pattern,index,minMax,syntax);
        if (newIndex > index) {
          if (minMax.first > minMax.second)
            throw new REException("an interval's minimum is greater than its maximum",REException.REG_BADRPT,newIndex);
          if (currentToken == null)
            throw new REException("quantifier (?*+{}) without preceding token",REException.REG_BADRPT,newIndex);
          if (currentToken instanceof RETokenRepeated) 
            throw new REException("attempted to repeat a token that is already repeated",REException.REG_BADRPT,newIndex);
          if (currentToken instanceof RETokenWordBoundary || currentToken instanceof RETokenWordBoundary)
            throw new REException("repeated token is zero-width assertion",REException.REG_BADRPT,newIndex);
          if ((currentToken.getMinimumLength() == 0) && (minMax.second == Integer.MAX_VALUE))
            throw new REException("repeated token may be empty",REException.REG_BADRPT,newIndex);
          index = newIndex;
	...
	char lastChar = 0;
	if (index == pLength) throw new REException("unmatched bracket",REException.REG_EBRACK,index);
	
	// Check for initial caret, negation
	if ((ch = pattern[index]) == '^') {
	  negative = true;
	  if (++index == pLength) throw new REException("unexpected end of character class",REException.REG_EBRACK,index);
	  ch = pattern[index];
	}
...
//	    break;
//          default:
            throw new REException("quantifier (?*+{}) without preceding token", REException.REG_BADRPT, index);
	  }
	}

	if (index >= pLength) {
	    throw new REException("unmatched parenthesis", REException.REG_ESUBREG,index);
	}
	...
	while ( ((nextIndex = getCharUnit(pattern,endIndex,unit)) > 0)
	  ...
	  if ((endIndex = nextIndex) >= pLength)
	    throw new REException("unexpected end of subexpression",REException.REG_ESUBREG,nextIndex);
	  else if (unit.ch == '(' && (syntax.get(RESyntax.RE_NO_BK_PARENS) ^ unit.bk))
	    nested++;
	  else if (unit.ch == ')' && (syntax.get(RESyntax.RE_NO_BK_PARENS) ^ unit.bk))
	    nested--;
	...
	throw new REException("unmatched parenthesis",REException.REG_EPAREN,index);
      }
	...
	if (currentToken == null)
          throw new REException("quantifier (?*+{}) without preceding token",REException.REG_BADRPT,index);
	if (currentToken instanceof RETokenRepeated)
          throw new REException("attempted to repeat a token that is already repeated",REException.REG_BADRPT,index);
	if (currentToken instanceof RETokenWordBoundary || currentToken instanceof RETokenWordBoundary)
	  throw new REException("repeated token is zero-width assertion",REException.REG_BADRPT,index);
	if (currentToken.getMinimumLength() == 0)
	  throw new REException("repeated token may be empty",REException.REG_BADRPT,index);
	...
	if (currentToken == null) throw new REException("quantifier (?*+{}) without preceding token",REException.REG_BADRPT,index);
  ...
  private static int getCharUnit(char[] input, int index, CharUnit unit) throws REException {
	...
	unit.ch = input[index++];
      else throw new REException("backslash at end of pattern",REException.REG_ESCAPE,index);
    return index;
  }
  ...
  private static REToken setRepeated(REToken current, int min, int max, int index) throws REException {
    if (current == null) throw new REException("quantifier (?*+{}) without preceding token",REException.REG_BADRPT,index);
  ...
  private int getMinMax(char[] input,int index,IntPair minMax,RESyntax syntax) throws REException {
    ...
    if (index == input.length) {
      if (mustMatch)
        throw new REException("unmatched brace",REException.REG_EBRACE,index);
      else
        return startIndex;
    }
    ...
    if (buf.length() == 0) {
      if (mustMatch)
        throw new REException("an interval is empty * or contains illegal characters",REException.REG_EBRACE,index);
      else
        return startIndex;
    }
    ...
    if ((unit.ch == '}') && (syntax.get(RESyntax.RE_NO_BK_BRACES) ^ unit.bk))
      ...
      if (mustMatch)
        throw new REException("unexpected end of interval",REException.REG_EBRACE,index);
      else
        return startIndex;
    else if ((unit.ch == ',') && !unit.bk) {
