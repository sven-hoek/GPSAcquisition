...
public final class Character implements Serializable, Comparable
{
   ...
   * @since 1.1
   */
	// TODO: Get primitive class for C
  ...
  static char readChar(char ch)
  {
    // Perform 16-bit addition to find the correct entry in data.
	  System.out.println("CCCCCCH: " + (int)ch + " "  + (int)(ch >> CharData.SHIFT) + " " + data.length);
...
//    return isJavaIdentifierPart(ch);
  ...
  {
    int category =0;// getType(ch);
    return ((1 << category) & ((1 << UPPERCASE_LETTER) | (1 << LOWERCASE_LETTER) | (1 << TITLECASE_LETTER) | (1 << MODIFIER_LETTER) | (1 << OTHER_LETTER) | (1 << NON_SPACING_MARK)
