...
/**
 * Invokes a nonvirtual Method (e.g. private object method)
 * @author jung
 *
 */
public class InvokeNonVirtualInstr extends Instruction {

	public InvokeNonVirtualInstr(I instr, Integer pos) {
		super(instr, pos);
	}
	...
	public void eval(SynthData data) {
		...
		int functionID = (Integer)i().getByteCodeParameter(data);
		if(data.kownIntrinsic(functionID)){ // Check whether this function is a known Intrinsic of the CGRA
			...
			for(int i = 0; i< paramCount; i++){ // Pop all parameters from the stack
